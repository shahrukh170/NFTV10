...
class NetworkTopo( Topo ):
    ...
    def build( self, **_opts ):
        ...
        defaultIP = '192.168.1.2/24'  # IP address for r0-eth1
        ...
        self.addLink( s3, router, intfName2='s3-eth0',
                      params2={ 'ip' : '172.16.1.1/24' } )
        self.addLink( s4, router, intfName2='s4-eth0',
                      params2={ 'ip' : '172.16.1.4/24' } )

        h1 = self.addHost( 'h1', ip='192.168.1.100/24',
                           defaultRoute='via 192.168.1.2' )
        h2 = self.addHost( 'h2', ip='192.168.2.200/24',
                           defaultRoute='via 192.168.2.3' )
        h3 = self.addHost( 'h3', ip='172.16.1.100/24',
                           defaultRoute='via 192.168.1.2' )
        h4 = self.addHost( 'h4', ip='172.16.2.200/24',
                           defaultRoute='via 192.168.2.3' )
        
        for h, s in [ (h1, s1), (h3, s1), (h2, s2),(h4 , s2),(h3 , s3),(h4 , s4) ]:
            self.addLink( h, s )
...
def run():
    ...
    net.start()
    #info(net['r0'].cmd("ip route add 192.168.1.0/24 via 192.168.1.2 dev eth1"))
    #info(net['r0'].cmd("ip route add 192.168.2.0/24 via 192.168.1.2 dev eth2"))
    #info(net['r0'].cmd("ip route add 172.16.1.0/24 via 192.168.2.3 dev eth1"))
    #info(net['r0'].cmd("ip route add 172.16.2.0/24 via 192.168.8.103 dev eth2"))
    ...
    command4 = 'sudo ./NELphase-master/nel sender 127.0.0.1 192.168.1.100'
