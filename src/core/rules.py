...
PACKET_BASED_FILTER_RULES = {
	1:('--type IP --condition flags==128,version==4 --protocol tcp --action drop',"drop all TCP IPv4 packets w/ reserved flag set"),
	2:('--type IP --condition id==9999,version==4 --protocol tcp --action drop','drop all TCP IPv4 packets w/ id = 9999'),
	3:('--type IP --condition tos==3 --protocol udp --action forward',"forward all UDP ip packets with tos == 3"),
	4:('--type IP --condition flags==32,frag==18 --protocol tcp --action drop','drop all TCP IP packets with DF set'),
...
class Rule(object):
	  ...
	  utility methods
        ...
        def getPacketBasedFilterRuleByNumber(rule_no):
                ...
                r_parser = argparse.ArgumentParser(description = "Rule parser" )
		...
		#print("    ##" + (" parsing rule #" + str(rule_no)).ljust(30,'.'), end = '...')
                ...
                if result.conds == '':
                        return Rule(num=rule_no, ptype=result.ptype, action=result.action, \
                               protocol=result.protocol.upper(),norm_op=norm_op_str, desc=rule[1])

                conds_str = ' '.join(result.conds)
                return Rule(num=rule_no, ptype=result.ptype, conds=conds_str.split(','), action=result.action, \
                        protocol=result.protocol.upper(),norm_op=norm_op_str, desc=rule[1])
	  ...
	  get a given no of random rules, and the rules should not be in the initial list given as a parameter to the method
        ...
        def get_rules_from_nums(rule_nums):
		  ...
		  Create Rule objects from rule numbers
                ...
                for rule_num in rule_nums:
                        rl = Rule.getPacketBasedFilterRuleByNumber(rule_num)
                        if rl != None:
                                rules.append(rl)
                                init += 1
                        else:
        ...
        @staticmethod
        def get_all_rules():
                return PACKET_BASED_FILTER_RULES.items()
