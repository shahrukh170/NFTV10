...
PACKET_BASED_FILTER_RULES = {
	1:('--type IP --condition flags==128,version==4 --protocol tcp --action drop',"drop all IPv4 packets w/ reserved flag set"),
	2:('--type IP --condition id==9999,version==4 --protocol tcp --action drop','drop all IPv4 packets w/ id = 9999'),
	3:('--type IP --condition tos==3 --protocol tcp --action forward',"forward all ip packets with tos == 3"),
	4:('--type IP --condition flags==32,frag==18 --protocol tcp --action drop','drop all IP packets with DF set'),
	...
	17:('--type IP --condition flags==32,sport==1234 --protocol tcp --action drop','drop all TCP packets with DF flag set and sport == 1234'),
